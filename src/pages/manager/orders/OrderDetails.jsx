    ...
    <Container sx={{ pt: 4 }}>
      <MainCard sx={{ maxWidth: "65%" }}>
        <Grid container spacing={3}>
          <Grid item xs={4}>
            <Typography variant="h4">Pedido #{order.code}</Typography>
          </Grid>
          <Grid item xs={4}>
            <OrderStatus status={order.status} variant="h4" />
          </Grid>
          <Grid item xs={4}>
            <Typography variant="h4">{new Date(order.createdAt).toLocaleString()}</Typography>
          </Grid>
        </Grid>
        <Divider sx={{ my: 3 }} />
        <Grid container>
          ...
          <Grid item xs={6} display="flex" alignItems="center">
            <Typography variant="h4">Total a pagar ${order.amount}</Typography>
          </Grid>
        </Grid>
        <Divider sx={{ my: 3 }} />
        <Grid container spacing={3}>
        ...
        <Grid container spacing={1} alignItems="center">
          ...
          <Grid item xs={3}>
            <FormControl fullWidth>
              ...
              <Select labelId="status-select-label" id="status-select" label="Estado" value={status} onChange={(e) => setStatus(e.target.value)}>
